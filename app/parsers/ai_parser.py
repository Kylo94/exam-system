...
class AIParser:
    ...
    def batch_enhance_questions(self, questions: List[Dict[str, Any]], image_info: List[Dict] = None, existing_kps: List[str] = None) -> List[Dict[str, Any]]:
        ...
        prompt = self._build_batch_prompt(questions, image_info, existing_kps)
...
9. 如果现有知识点都不匹配且题目涉及新概念，可以建议创建新知识点（格式："新建:知识点名称"）"""
        ...
        try:
            ...
            logger.warning(f"Batch enhance failed: {e}")
            return self._preserve_local_results(questions)

    def _build_batch_prompt(self, questions: List[Dict[str, Any]], image_info: List[Dict] = None, existing_kps: List[str] = None) -> str:
        """构建批量处理的提示词（简化版，一次完成格式标准化+知识点匹配）"""
        images_by_question = {}
        if image_info:
        ...
        prompt = f"请解析以下 {len(questions)} 道题目，返回标准JSON数组（必须包含所有题目，保持顺序一致）：\n\n"
...
"""
        ...
        kp_list = existing_kps[:50] if existing_kps else []  # 限制50个知识点
        kp_str = ", ".join(kp_list) if kp_list else "（暂无已有知识点，将自动创建）"
...
2. 判断题（true_false）无选项，correct_answer为"true"或"false"
3. 每道题必须分配知识点，使用已有知识点名称，或返回"新建:知识点名"创建新知识点
4. 直接返回JSON数组，不要任何其他文字，不要用markdown包裹

返回格式：
[{{"order":1,"type":"true_false","content":"...","correct_answer":"true","knowledge_point_names":["existingKP"],"explanation":"..."}},...]
"""
    ...
    def _merge_local_and_ai_results(self, local_questions: List[Dict], ai_results: List[Dict], image_info: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for idx, local_q in enumerate(local_questions):
            ...
            if local_type == 'true_false':
                logger.debug(f"判断题 detected, keeping type as true_false, AI type was: {ai_q.get('type') if ai_q else 'None'}")
            ...
            if ai_q and isinstance(ai_q, dict):
                ...
                # 保留AI返回的知识点名称
                kp_names = ai_q.get('knowledge_point_names')
                if kp_names is not None:
                    result['knowledge_point_names'] = kp_names
                    logger.debug(f"题{idx+1} AI返回知识点: {kp_names}")
