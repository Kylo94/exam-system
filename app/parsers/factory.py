...
class ParserFactory:
    ...
    def parse_file(cls, file_path: str) -> Dict[str, Any]:
        ...
        if not parser:
            supported = ', '.join(cls.get_supported_formats().keys())
    ...
    def parse_to_exam(cls, file_path: str, exam_title: str,
        ...
        # 检查解析器是否有 parse_to_exam 方法
        if hasattr(parser, 'parse_to_exam'):
            return parser.parse_to_exam(file_path, exam_title, subject_id, level_id)
        
        # 否则使用通用方法
        parsed_data = parser.parse(file_path)
        questions = parser.extract_questions(parsed_data)
        ...
        exam_data = {
            ...
            'level_id': level_id,
            'description': f"从文档 '{Path(file_path).name}' 导入的考试",
            'duration_minutes': len(valid_questions) * 2,
